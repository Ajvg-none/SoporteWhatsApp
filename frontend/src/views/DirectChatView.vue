...
<template>
  <div class="animate-fadeIn h-[calc(100vh-8rem)] flex flex-col">
    ...
    <div class="flex items-center justify-between mb-4 shrink-0">
      <div>
        <h1 class="text-2xl font-black text-body tracking-tight">
          💬 Chat Privado VIP
        </h1>
        <p class="text-secondary mt-1 text-sm font-semibold">
    ...
    <!-- Chat con Sidebar -->
    <div class="flex-1 bg-surface-card rounded-2xl border border-edge shadow-sm flex overflow-hidden min-h-0">
      
      <!-- Sidebar: Lista de números VIP -->
      <aside class="w-64 border-r border-edge flex flex-col shrink-0 bg-surface-hover">
        <!-- Header del sidebar -->
        <div class="p-4 border-b border-edge bg-surface-card shrink-0">
        ...
        <div class="flex-1 overflow-y-auto">
          <div
            ...
            class="p-3 hover:bg-surface-hover cursor-pointer transition-all duration-150 border-b border-edge"
            :class="{
              'bg-primary-light dark:bg-primary-deep border-l-4 border-l-primary': selectedNumber === chat.numero,
          ...
          >
            <div class="flex items-start gap-2.5">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center justify-between">
                  <span class="text-sm font-bold text-body truncate">
                    {{ chat.nombre || chat.numero }}
                  </span>
                  <span class="text-[10px] font-semibold text-muted whitespace-nowrap ml-2">
                    {{ formatTime(chat.ultimoEnvio) }}
                  </span>
                </div>
                <div class="flex items-center justify-between mt-0.5">
                  <span class="text-xs text-secondary truncate flex-1">
          ...
          <div v-if="chatNumbers.length === 0" class="text-center py-12">
            ...
            <p class="text-sm font-bold text-secondary">Sin chats privados</p>
            <p class="text-xs text-muted mt-1">Agrega números como "Chat Privado"</p>
          </div>
        </div>
      </aside>
      ...
      <main class="flex-1 flex flex-col min-h-0">
        ...
        <template v-if="selectedNumber">
          <!-- Header del chat -->
          <div class="px-6 py-3 border-b border-edge bg-surface-card shrink-0 flex items-center gap-3">
            ...
            <div>
              ...
              <div class="text-[10px] text-muted font-semibold">
          ...
          <!-- Lista de mensajes -->
          <div ref="chatContainer" class="flex-1 overflow-y-auto p-6 space-y-4 bg-surface-hover">
            <div
            ...
            >
              ...
              >
                <div class="text-[10px] font-extrabold tracking-wider uppercase mb-1.5" :class="msg.remitente === 'supervisor' ? 'text-white/75' : 'text-muted'">
                ...
                <div
                  ...
                  :class="msg.remitente === 'supervisor' ? 'border-white/15' : 'border-edge dark:border-edge'"
                >
                  <img
                ...
                <div class="text-[10px] text-right mt-1.5 font-bold uppercase tracking-wider" :class="msg.remitente === 'supervisor' ? 'text-white/50' : 'text-muted'">
            ...
            <div v-if="selectedChatMessages.length === 0" class="text-center py-16">
              ...
              <p class="text-secondary font-bold text-sm">Sin mensajes</p>
              <p class="text-muted text-xs font-semibold mt-1">Este chat está vacío</p>
            </div>
          </div>
          ...
          <div class="p-4 border-t border-edge bg-surface-card shrink-0">
            <form @submit.prevent="handleSendMessage" class="flex items-end gap-2">
              ...
              <button
                ...
                class="inline-flex items-center justify-center w-10 h-10 shrink-0 rounded-xl bg-input text-secondary hover:text-primary hover:bg-primary/10 transition-all cursor-pointer"
              >
              ...
              <div class="flex-1 flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  rows="1"
                  class="w-full px-4 py-2.5 bg-input border border-edge rounded-xl text-sm focus:outline-none focus:ring-4 focus:ring-primary/10 focus:border-primary text-body transition-all duration-200 placeholder-gray-400 dark:placeholder-gray-300 min-h-[40px] max-h-[120px] resize-none"
                  @input="autoResize"
        ...
        <template v-else>
          <div class="flex-1 flex items-center justify-center flex-col p-8 text-center">
            ...
            <h3 class="text-lg font-bold text-secondary">Selecciona un chat</h3>
            <p class="text-sm text-muted mt-1">Elige una conversación del panel izquierdo</p>
          </div>
        </template>
      </main>
    </div>
